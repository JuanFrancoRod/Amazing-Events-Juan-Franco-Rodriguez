...
let eventos =  filtroFecha (data.eventos)
let carta = ``
...
function filtroFecha(listaDeEventos){
...
fetch("https://mindhub-xj03.onrender.com/api/amazing")
  ...
  .then((res) => {
    ...
    ImprimirCard(upcomingEvent, $contcards);
...
function aplicarFiltro() {
  ...
  let filtroDeEventos;
  if (checked.length == 0) {
    filtroDeEventos = upcomingEvent;
  } else {
    filtroDeEventos = upcomingEvent.filter((event) =>
